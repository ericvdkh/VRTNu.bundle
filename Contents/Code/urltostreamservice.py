...
class UrlToStreamService:
    ...
    def get_stream_from_url(self, url):
        if "Cookies" in Dict:
            ...
            #mpd = self.get_mpegdash(stream_json['targetUrls']).replace("https", "http")  

            streamMetadata = helperobjects.StreamMetadata(hls) #hls) #mpd)
    ...
    @staticmethod
    def get_mpegdash(dictionary):
        for item in dictionary:
            if item['type'] == 'MPEG_DASH':
                return item['url']
